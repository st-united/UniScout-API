...
export class FileExportService {
  ...
  async generateExcel(data: UniEntity[], filename: string): Promise<string> {
    try {
      ...
      worksheet.columns = [
        ...
        // Add more columns as needed based on UniEntity
        { header: 'Agricultural & Food Science', key: 'agriculturalFoodScience', width: 10 },
      ...
      data.forEach((uni) => {
        worksheet.addRow({
          ...
          agriculturalFoodScience: uni.agriculturalFoodScience ? 'Yes' : 'No',
          artsDesign: uni.artsDesign ? 'Yes' : 'No',
          economicsBusinessManagement: uni.economicsBusinessManagement ? 'Yes' : 'No',
          engineering: uni.engineering ? 'Yes' : 'No',
          lawPoliticalScience: uni.lawPoliticalScience ? 'Yes' : 'No',
          medicinePharmacyHealthSciences: uni.medicinePharmacyHealthSciences ? 'Yes' : 'No',
          physicalScience: uni.physicalScience ? 'Yes' : 'No',
          socialSciencesHumanities: uni.socialSciencesHumanities ? 'Yes' : 'No',
          sportsPhysicalEducation: uni.sportsPhysicalEducation ? 'Yes' : 'No',
          technology: uni.technology ? 'Yes' : 'No',
          theology: uni.theology ? 'Yes' : 'No',
        });
      });
  ...
  async generatePdf(data: UniEntity[], filename: string): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      // Table Headers and their predefined widths
      ...
      currentY += headerHeight; // Move Y position down after header
